...
export const createAuthorizationHeader = async (message: any) => {
    ...
    const header = `Signature keyId="${subscriber_id}|${getConfig().app.uniqueKey}|ed25519",algorithm="ed25519",created="${created}",expires="${expires}",headers="(created) (expires) digest",signature="${signature}"`
    return header;
}
...
export async function verifyHeader(header: string, req: Request, res: Response) {
    try {
        const parts = split_auth_header(header);
        if (!parts || Object.keys(parts).length === 0) {
            throw new Exception(ExceptionType.Authentication_HeaderParsingFailed, "Header parsing failed", 401);
        }
